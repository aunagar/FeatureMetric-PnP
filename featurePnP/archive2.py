...
def interpolate_tensor(tensor, pts):
    '''
    Assume that the points are already scales to the size of the tensor (Really? ^^)
    tensor: C x H x W
    pts: N x 2
    return N x C
    '''
    print(pts)
    c, h, w = tensor.shape
    pts_downscale = (pts / pts.new_tensor([w-1, h-1])) * 2 -1
    args = {'align_corners': True} if int(torch.__version__[2]) > 2 else {}
    print(pts_downscale)
    tensor_out = torch.nn.functional.grid_sample(
        tensor[None], pts_downscale[None, None], mode='bilinear', **args)
