...
def indexing_(feature_map, points_, width, height):
    '''
    ...
    if points_.dtype != torch.int:
        points = torch.round(points_).type(torch.IntTensor) -1
    else:
        points = points_
...
# New indexing_
def interpolate_tensor(tensor, pts, im_size=(1024,1024), old =True):
    '''
    ...
    c, h, w = tensor.shape
    if old:
        return indexing_(tensor, torch.flip(pts,(1,)), im_size[0], im_size[1])
...
def mask_in_image(pts, w,h, pad=1):
...
class sparseFeaturePnP(nn.Module):
    ...
    def compute_cost(self, pts3D, R, t, feature_map_query, feature_ref, K,im_width, im_height):
        ...
        # Get the points that are supported within our image size
        mask_supported = mask_in_image(points_2d,im_width, im_height)
    ...
    def forward(self, pts3D, feature_ref, feature_map_query,
        ...
        for i in range(self.iterations):
            ...
            print(points_2d_supported)
            print(interpolate_tensor(feature_map_query, points_2d_supported, im_size=(im_width, im_height)))
            error = interpolate_tensor(feature_map_query, points_2d_supported, im_size=(im_width, im_height)) - feature_ref[mask_supported]
            #error = indexing_(feature_map_query, torch.flip(points_2d_supported,(1,)), im_width, im_height) - feature_ref[mask_supported]
            if self.use_ratio_test_:
                cost = 0.5 * (error**2).sum(-1) # Why 0.5??
            ...
            grad_x_points = interpolate_tensor(feature_grad_x, points_2d_supported, im_size=(im_width, im_height))
            grad_y_points = interpolate_tensor(feature_grad_y, points_2d_supported, im_size=(im_width, im_height))
            ...
            new_error = interpolate_tensor(feature_map_query, new_points_2d_supported, im_size=(im_width, im_height))- feature_ref[mask_supported]
            if self.use_ratio_test_:
