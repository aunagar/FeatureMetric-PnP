...
def optimize(query_hypercolumns, net, prediction, K, image_shape):
	...
	reference_hypercolumns = net.compute_hypercolumn(
                [prediction.reference_filename], to_cpu=False, resize=True)
	relative_shape = np.array([(image_shape[0]/reference_hypercolumns.shape[2]).astype(float),
								(image_shape[1]/reference_hypercolumns.shape[3]).astype(float)])
